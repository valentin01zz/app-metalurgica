...
class ReciboFactura:
    def __init__(self, root):
        ...
        self.campos = {}
        for campo in ["Nombre del Cliente", "Numero de Teléfono del Cliente", "Descripcion", "Ancho (en metros)", "Alto (en metros)", "Cantidad de la medida", "Precio de la medida ingresada","Porcentaje de Mano de Obra"]:
            subframe = CTkFrame(self.frame_presupuesto, fg_color="transparent")  # marco transparente
            subframe.pack(pady=10, anchor="center", fill="x")  # centrado y ocupa todo el ancho
            label = CTkLabel(subframe, text=campo + ":", width=200, anchor="e")  # ancho fijo y texto alineado a la derecha
            label.pack(side="left", padx=10)
            entrada = CTkEntry(subframe, width=250)
    ...
    def agregar_medida(self):
        try:
            ...
            largo = self.campos["Alto (en metros)"].get().replace(".",",")
            ...
            self.campos["Alto (en metros)"].delete(0, 'end')
    ...
    def calcular_presupuesto(self):
        try:
            ...
            mano_obra = float(self.campos["Porcentaje de Mano de Obra"].get())
            ...
            if len(self.__medidasyprecios) == 0:
                ...
                generar_pdf(nombre, telefono,tipo_persiana,desc,self.__medidasyprecios,mano_obra)
            self.__medidasyprecios = []  
        except ValueError:
            messagebox.showerror("Error", "Por favor, ingrese valores válidos.")
